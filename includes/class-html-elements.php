...
class MOSS_HTML_Elements {
	...
	public function month_dropdown( $name = 'month', $selected = 0 ) {
		...
		while ( $month <= 12 ) {
			$options[date('m', mktime(0, 0, 0, $month, 1))] = date("F", mktime(0, 0, 0, $month, 1));
			$month++;
		}
	...
	public function select( $args = array() ) {
		...
		$output = '<select name="' . esc_attr( $args[ 'name' ] ) . '" id="' . esc_attr( sanitize_key( str_replace( '-', '_', $args[ 'id' ] ) ) ) . '" class="vat-moss-select ' . esc_attr( $args[ 'class'] ) . '"' . $multiple . '>';
