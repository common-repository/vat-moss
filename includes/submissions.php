...
// On some Linux distrubutions the locale can be > 255 characters which is no allowed and raised a warning
// Instead, look for the LC_ALL value but if not found pass an array of the values.
if ( strlen( $locale ) > 255 )
{
	$locale = explode(";", $locale );
	$loc = array();
	foreach( $locale as $bit )
	{
	   $value = explode("=",$bit);
	   $loc[ $value[0] ] = $value[1];
	}
	$locale = isset( $loc['LC_ALL'] )
		? $loc['LC_ALL']
		: $loc;
}
...
{
	advert( 'wordpress-vat-moss-reporting', VAT_MOSS_VERSION, function() {
