...
function new_submission($from_year = null, $from_month = null, $to_year = null, $to_month = null, $submission_id = 0, $read_only = false)
{
	...
	advert( 'wordpress-vat-moss-reporting', VAT_MOSS_VERSION, function() use( $from_year, $from_month, $to_year, $to_month, $submission_id, $read_only ) {
...
<?php	if ( $read_only ) { ?>
											...
											<input type="hidden" id="ecsl_settings_test_mode" value="<?php echo esc_attr( $test_mode ); ?>">
<?php	} else { ?>
...
<?php	} ?>
									...
									<tr>
										...
										<td>
<?php	if ( $read_only ) { ?>
											<span><?php echo esc_attr( $submission_key ); ?></span>
<?php	} else { ?>
											<input type="text" class="regular-text" id="submission_key" name="submission_key" value="<?php echo esc_attr( $submission_key ); ?>">
<?php	} ?>
										</td>
									</tr>
<?php	if ( ! $read_only ) { ?>
